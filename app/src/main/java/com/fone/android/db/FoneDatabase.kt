...
abstract class FoneDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        fun getDatabase(context: Context): FoneDatabase {
            synchronized(lock) {
                if (INSTANCE == null) {
                    INSTANCE = Room.databaseBuilder(context, FoneDatabase::class.java, Constants.DataBase.DB_NAME)
        ...
        fun getReadDatabase(context: Context): FoneDatabase {
            synchronized(readlock) {
                if (READINSTANCE == null) {
                    READINSTANCE = Room.databaseBuilder(context, FoneDatabase::class.java, Constants.DataBase.DB_NAME)
                        .enableMultiInstanceInvalidation()
                        .build()
                }
